...
export const SubmitOfferScreen: React.FC = ({ route, navigation }: any) => {
  ...
  const currentRequest = rideRequests.find((r) => r.id === rideRequestId);
  ...
  const onSubmit = async (data: OfferForm) => {
    try {
      ...
      await createConversation(rideRequestId, currentRequest?.student_id || '', user.id);
